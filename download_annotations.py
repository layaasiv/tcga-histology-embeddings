...
def download_file(case_id: str, submitter_id: str, project_id: str):
    ...
    file_path = Path(save_path, f"{file_name}.svs")
    if file_path.exists():
        print(f"{file_name}.svs already exists. Skipping download.")
        return
...
def main():
    ...
    # download_file(combined_ids[0][0], combined_ids[0][1], combined_ids[0][2])
